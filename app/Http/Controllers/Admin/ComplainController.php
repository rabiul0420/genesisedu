...
class ComplainController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['complain_info'] = DoctorComplain::where('status', '!=', 0)->orderBy('id', 'DESC')->get();
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()){
            ...
            return redirect()->action('Admin\CmplpainController@edit')->withInput();
        }

            $reply = new DoctorComplainReply();
            ...
            $complain = DoctorComplain::find($request->complain_id);
            $complain->status = 2;
            $complain->push();
    ...
    public function edit($id)
    {
        ...
        $complain_id=DoctorComplain::find($id);
        $doctor_name = Doctors::get()->where('id',$complain_id)->pluck('name', 'id');

        //$complain_replied = DoctorComplainReply::where('complain_id', $complain_id->id)->get();
        $complain_replied = DoctorComplainReply::where('complain_id', $complain_id->id)->orderBy('id', 'ASC')->get();
        ...
        $array_data = array(
            ...
            'complain_replied'=>$complain_replied,
            'title'=>$title,
            'doctor_name'=>$doctor_name,
        );
        //echo $array_data['complain_id'];
        return view('admin.complain.edit', $array_data);
    }
