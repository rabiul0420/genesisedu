        ...
        <div class="col-md-12">
            ...
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-globe"></i> Add Exams in {{ $topic->name }}
                        ...
                        @endcan
                        @can('Topic Content')
                        <a href="{{url('admin/topic-exam-list/'.$topic->id)}}" class="btn btn-xs btn-success">Topic Exams</a>
                        @endcan
                    </div>
                </div>
...
<link href="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.3/css/select2.min.css" rel="stylesheet" />
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '[name="exam_id"]', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: '/admin/topic-exam-save',
                    dataType: 'HTML',
                    data: {exam_id : exam_id, topic_id : topic_id, operation : operation },
                    success: function( data ) { 
                        var data = JSON.parse(data);
                        $("#label_"+data['exam_id']).html(data['message']);
                        
                        if(data['status'] == "insert_success")
                        {
                            $("#label_m_"+data['exam_id']).removeClass("btn btn-info").addClass("btn btn-danger");
                            $("#label_m_"+data['exam_id']).html("Delete From Topic");
                        }
                        if(data['status'] == "delete_success")
                        {
                            $("#label_m_"+data['exam_id']).removeClass("btn btn-danger").addClass("btn btn-info");
                            $("#label_m_"+data['exam_id']).html("Add to Topic");
                        }
                                                            
                        if(data['status'] == "completed" || data['status'] == "data_already_exist" )
                        {
                            window.location.href = "/admin/topic-exam-list/"+data['topic_id'];
                        }
