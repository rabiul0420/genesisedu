        ...
        <div class="col-md-12" id="application">
            ...
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption"><i class="fa fa-reorder"></i>{{ $batch_schedule->name }}</div><span><input type="button" class="btn btn-info" style="margin-left: 30px; margin-top: -9px;" onclick="printDiv('printableArea')" value="Print Schedule" /></span>
                </div>
                    ...
                    {{-- @if( $batch_schedule->time_slots instanceof \Illuminate\Support\Collection )
                        @foreach( $batch_schedule->time_slots as $slot  )
                            ...
                            <div style="margin-bottom: 30px">
                                <div style="border-bottom: 1px dashed darkred">{{$slot->datetime->format('d - F Y l h:i a')}}</div>
                                <div>
                                    @if( $slot->schedule_details instanceof \Illuminate\Support\Collection )
                                        @foreach( $slot->schedule_details as $class  )
                                            <div>
                                                <span class="badge badge-success">{{ $class->type  }}</span>
                                                {{ $class->type == 'Exam' ? ($class->exam->name ?? '') : ( $class->video->name ?? '')  }}
                                                {{ $class->mentor->name ?? ''  }}

                                                @if( $class->lectures instanceof \Illuminate\Support\Collection )
                                                    <div style="margin-left: 10px">
                                                        @foreach( $class->lectures as $subClass  )
                                                            <div>
                                                                <span class="badge badge-{{ $class->type== "Class" ? "warning" : "info"  }}">
                                                                    {{ $class->type == 'Class' ? 'Feedback Class':'Solve Class'  }}
                                                                </span>
                                                                {{ $subClass->video->name ?? '' }}
                                                                {{ $subClass->mentor->name ?? ''  }}
                                                            </div>
                                                        @endforeach
                                                    </div>
                                                @endif
                ...
                {{-- <div class="text-center"><input type="button" class="btn btn-primary" onclick="printDiv('printableArea')" value="Print" /></div> --}}
