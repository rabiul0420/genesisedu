                ...
                <div class="portlet-body form">
                    <div class="panel panel-primary" style=" border-color: #eee;" >
                        ...
                        <div class="panel-body">
                            ...
                            <table class="table table-bordered table-striped routine-rooms table_routine">
                                    ...
                                    @foreach ($batch_slots as $value)
                                        @if ($value['slot_type'])
                                            ...
                                            @if ($value['slot_type'] == '3')
                                                <th style="vertical-align: middle !important;text-align: center;border-left-width: 1px;border-bottom-width: 1px;" width="15%" rowspan="{{($count_rows * $count_slots_all + 1)}}">{{ $value->slot->slot_name }} <br> {{ $value['start_time'] }} TO {{ $value['end_time'] }} </th>
                                            @else
                                ...
                                @if(!empty($topics_list))
                                    ...
                                    @foreach($topics_list as $key=>$topics)
                                        @if($topics=='Select Class/Chapter')
                                            @continue
                                        @endif
                                        <tr>
                                            @for($class_count_C = 1 ;$class_count_C<=($count_slots+1);$class_count_C++)
                                                @if($class_count_C=='1')
                                                    <td  style="border-left-width: 1px;border-bottom-width: 1px;">
                                                        <div class="">
                                                            <div style="text-align:right;" data-date-viewmode="years" data-date-format="yyyy-mm-dd" data-date="{{ date("Y-m-d", strtotime($schedule_details->initial_date)) }}" class="">
                                                                {{--<input type="text"  name="schedule_dateggg" id="{{'schedule_date_main'.'__1'.'_'.$i.'_'.$class_count_C }}" onchange="change_df_value(this.id,this.value,{{ $count_slots+1 }})" value="{{date("Y-m-d", strtotime("+$day_increament day", strtotime($schedule_details->initial_date)))}}" size="16" class="form-control input-append date">
                                                                --}}
                                                                <input type="text"  name="schedule_dateggg" id="{{'schedule_date_main'.'__1'.'_'.$r.'_'.$class_count_C }}" onchange="change_df_value(this.id,this.value,{{ $count_slots+1 }})" value="{{ $schedule_dates[$d] }}" size="16" class="form-control input-append date">

                                                            </div>
                                                            <br>
                                                            <div id="{{ 'date_day__1' . '_' . $r . '_' . $class_count_C }}" style="text-align:center;">
                                                ...
                                                @elseif($m_slot_lists[$class_count_C-1]==1 || $m_slot_lists[$class_count_C-1]==2)
                                                    <td  style="border-left-width: 1px;border-bottom-width: 1px;">
                                                        @php $data = array('type'  => 'hidden',
                                                                        'id'    => 'schedule_date'.'__1'.'_'.$r.'_'.$class_count_C,
                                                                        'class' => ''
                                                                        );
                                                        @endphp
                                                        {{--{!! Form::hidden('schedule_date[]', date("Y-m-d",strtotime("+$day_increament day",strtotime($schedule_details->initial_date))), $data) !!}--}}
                                                        {!! Form::hidden('schedule_date[]', $schedule_dates[$d], $data) !!}
                                                        ...
                                                        @endphp
                                                        {!! Form::hidden('slot_id[]',$m_slot_lists[$class_count_C-1],$data) !!}
                                                        @php unset($data) @endphp
                                                        @php
                                                            $data = array('type'  => 'hidden',
                                                                'id'    => '',
                                                        ...
                                                        @endphp
                                                        {!! Form::hidden('topics_id[]', $m_slot_lists[$class_count_C-1], $data) !!}
                                                        @php unset($data) @endphp
                                                        <div class="">
                                                            @php  $topics_list->prepend('Select Class/Chapter', '') @endphp
                                                            @if(($m_slot_lists[$class_count_C-1]==2))
                                                                @php
                                                                    $data = array('class' => 'form-control exam2  select2 ',
                                                                            'required' => 'required',
                                                                            'multiple'=>'multiple'
                                                                        );
                                                                @endphp
                                                                {!! Form::select('examtopic_'.$r.'[]', $topics_list,(isset($key) ? $key : '') ,$data) !!}
                                                                <input type="hidden" name="topic_id[]" value="{{( isset($key) ? $key : '') }}">
                                                            @elseif($m_slot_lists[$class_count_C-1]==1)
                                                                @php
                                                                    $data = array('class' => 'form-control   select2 ',
                                                                            'required' => 'required'
                                                                        );
                                                                @endphp
                                                                {!! Form::select('topic_id[]', $topics_list,(isset($key) ? $key : ''),$data) !!}
                                                            @endif
                                                        </div>
                                                        <br>
                                                        <div class="">
                                                            ...
                                                            @endphp
                                                            {!! Form::select('teacher_id[]', $teachers_list,'',$data) !!}
                                                        </div>
                                                    </td>
                                                @endif
                                            @endfor
...
@section('js')
    ...
    <script type="text/javascript">
        ...
        function change_df_value(element_id,value,column_count) {
            ...
            for(col_id=2;col_id<column_count+1;col_id++){
