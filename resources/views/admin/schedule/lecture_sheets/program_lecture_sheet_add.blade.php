...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li> Add Lecture Sheets in {{ $program->name }}</li>
        </ul>
    </div>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-globe"></i> Add Lecture Sheets in {{ $program->name }}
                        ...
                        @endcan
                        @can('Program Content')
                        <a href="{{url('admin/program-lecture-sheet-list/'.$program->id)}}" class="btn btn-xs btn-success">Program Lecture Sheets</a>
                        @endcan
                    </div>
                </div>
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '[name="topic_content_id"]', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: '/admin/program-lecture-sheet-save',
                    dataType: 'HTML',
                    data: {topic_content_id : topic_content_id, program_id : program_id, operation : operation },
                    success: function( data ) { 
                        ...
                        {
                            $("#label_m_"+data['topic_content_id']).removeClass("btn btn-info").addClass("btn btn-danger");
                            $("#label_m_"+data['topic_content_id']).html("Delete From Program");
                        }
                        if(data['status'] == "delete_success")
                        {
                            $("#label_m_"+data['topic_content_id']).removeClass("btn btn-danger").addClass("btn btn-info");
                            $("#label_m_"+data['topic_content_id']).html("Add to Program");
                        }
                                                            
                        if(data['status'] == "completed" || data['status'] == "data_already_exist" )
                        {
                            window.location.href = "/admin/program-lecture-sheet-list/"+data['program_id'];
                        }
            ...
            var table = $('.datatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "/admin/program-lecture-sheet-add-list",
                    type: 'GET',
                    data: function (d) {
