...
@section('content')
    ...
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li> Add Faculty in {{ $module->name }}</li>
        </ul>
    </div>
        ...
        <div class="col-md-12">
            ...
            <div class="portlet">
                <div class="portlet-title">
                    <div class="caption">
                        <i class="fa fa-globe"></i> Add Faculty in {{ $module->name }}
                        ...
                        @endcan
                        @can('Module Content')
                        <a href="{{url('admin/module-faculty-list/'.$module->id)}}" class="btn btn-xs btn-success">Module Faculty</a>
                        @endcan
                    </div>
                </div>
  ...
  <div class="modal fade" id="question" tabindex="-1" aria-labelledby="facultypleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="facultypleModalLabel">Question</h5>
...
<link href="https://cdnjs.cloudflare.com/ajax/libs/select2/4.0.3/css/select2.min.css" rel="stylesheet" />
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            $(document).on('change', '[name="faculty_id"]', function() {
                ...
                $.ajax({
                    type: "POST",
                    url: '/admin/module-faculty-save',
                    dataType: 'HTML',
                    data: {faculty_id : faculty_id, module_id : module_id, operation : operation },
                    success: function( data ) { 
                        var data = JSON.parse(data);
                        $("#label_"+data['faculty_id']).html(data['message']);
                        
                        if(data['status'] == "insert_success")
                        {
                            $("#label_m_"+data['faculty_id']).removeClass("btn btn-info").addClass("btn btn-danger");
                            $("#label_m_"+data['faculty_id']).html("Delete From Topic");
                        }
                        if(data['status'] == "delete_success")
                        {
                            $("#label_m_"+data['faculty_id']).removeClass("btn btn-danger").addClass("btn btn-info");
                            $("#label_m_"+data['faculty_id']).html("Add to Topic");
                        }
                                                            
                        if(data['status'] == "completed" || data['status'] == "data_already_exist" )
                        {
                            window.location.href = "/admin/module-faculty-list/"+data['module_id'];
                        }
            ...
            $("#table_1").on("click", 'td' , function () {
                ...
                var faculty_id = $(this).closest('tr').find('td').first().html();
                
                if(!isNaN(faculty_id) && $(this).index() != ( $(this).closest('tr').children('td').length - 1 ) )
                {
                    $('#question_answer .modal-body').load('/admin/get-question-details',{faculty_id : faculty_id, _token: '{{ csrf_token() }}'},function(){
            ...
            var table = $('.datatable').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "/admin/module-faculty-add-list",
                    type: 'GET',
                    data: function (d) {
            ...
            $("body").on("click",".btn_view",function(){
                ...
                $('.modal-body').load('/admin/question-view',{faculty_id: faculty_id,_token: '{{csrf_token()}}'},function(){
            ...
            $("body").on("click",".btn_log",function(){
                var faculty_id = $(this).attr('id');
                $('.modal-body').load('/admin/question-edit-log',{faculty_id : faculty_id, _token: '{{ csrf_token() }}'},function(){
