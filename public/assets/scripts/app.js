jQuery.prototype.input_hidden = function( hidden ){
    const container = $( this );
    const input_containers = container.find( 'input' );
    window.input_types =  window.input_types || [];
    input_containers.each( function (i, item) {
        if( !window.input_types.find( el => el.name ===  item.name ) ) {
            window.input_types.push({
                name: item.name,
                type: item.type,
            });
        }
        const input_element = window.input_types.find( el => el.name ===  item.name );
        if( hidden ) {
            item.type = 'hidden';
        } else {
            item.type = input_element.type;
        }
    });
    return container;
}
...
var App = function () {
    ...
    function select_all( event ){
        select_id = this;
        const id = select_id.dataset.target;
        const data = document.querySelector( id );
        if( data.tagName.toLowerCase() == 'select' ) {
            for ( let i =0; i < data.children.length; i++ ) {
                if( select_id.checked ){
                    data.children[i].setAttribute( 'selected', 'selected' );
                }else {
                    data.children[i].removeAttribute( 'selected' );
                }
            }
            $(id).trigger("change");
        }
    ...
    var handleSelect2AllSelector = function (){
        const all_checkbox = document.querySelectorAll('input.all-selector[type="checkbox"][data-target]')

        console.log( 'all_checkbox', all_checkbox );

        for (let i = 0; i <  all_checkbox.length; i++ ) {
            all_checkbox[i].addEventListener( 'change', select_all  )
        }
    }
    ...
    return {
        ...
        init: function () {
            ...
            handleSelect2AllSelector(); // handles bootstrap switch plugin
        },
